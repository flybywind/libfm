 ...
 *      对于浮点特征，需要离散化
 * 1.1.1:
 *      vid不能直接转换成长整型，因为在cpu new一个数组的时候，最多只能是uint类型
 */
using namespace std;
...
int main(int argc, char **argv) { 
	...
	try {
        ...
        auto one_hot_feature_hash_map = new std::unordered_map<std::string, uint>();
        auto one_hot_feature_hash_set = new std::unordered_set<uint>();
                ...
                for(size_t i = 1; i < colums.size(); ++i) {
                     ...
                     if (part.size() > 0) {
                        ...
                        if (one_hot_feature_hash_map->count(feature_value) == 0) {
                            uint hash_value = str_hash(feature_value, seed);
                            while (one_hot_feature_hash_set->count(hash_value) > 0) {
                                std::cout << "INFO: feature: " << feature_value <<", hash key = " << hash_value << "\n";
                                seed = rand();
                                std::cout << "      hash meet conflict, new seed = " << seed << "\n";
                ...
                if (line_buf_seg.size() > 1) {
                    ...
                    for (size_t i = 1; i < line_buf_seg.size(); ++i) {
                        ...
                        if (segs.size() > 1) {
                            ...
                            if (discrete_feature_range_map.count(feature_name) > 0) { // 浮点数，需要离散化的特征 
                                if (!discrete_feature_bypoints(discrete_feature_range_map,
                                        feature_name, feature_value)) {
                                    std::cerr<<"ERROR: discrete featrue "<<feature_name <<"failed at line ["
                                    << line_no<<"] , line is\n"<< line<<std::endl;
                                    exit(-1);
                                }
                            }
                            ...
                            oLibFM << " " << fid << ":1";
                        /*} else {
                            std::cerr << "WARNING: invalid colum `"<<i<<"`, of line ["<< line_no <<"], col = "
